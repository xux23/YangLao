...
> 版本：v1.1　｜　日期：2026-09-10　｜　配套：《需求分析文档.md》《系统设计文档.md》
>
> v1.1 变更：认证模块接入 Redis——新增图形验证码接口（2.1）、登录增加验证码参数与失败锁定（2.2）、新增安全退出接口（2.5）。
...
- 数据格式：JSON（UTF-8）
- 认证方式：除登录、图形验证码外，所有接口需携带请求头 `Authorization: Bearer <token>`
...
`GET /auth/captcha`　权限：无

说明：生成 4 位图形验证码（Hutool LineCaptcha），验证码文本以 `captcha:{captchaId}` 存入 Redis，有效期 5 分钟，**校验一次后立即删除（一次性）**。前端将 image 直接作为 `<img>` 的 src 展示，点击图片重新获取。

响应 data：

```json
{
  "captchaId": "8f3a9c2e-1b4d-4e6f-9a2b-3c5d7e8f9a0b",
...
| captchaId | string | 是* | 验证码编号（2.1 返回；`app.captcha.enabled=false` 时可省略） |
| captchaCode | string | 是* | 用户输入的验证码文本（不区分大小写） |

校验顺序与错误提示：

1. 锁定检查：该用户名登录失败次数已达上限（默认 5 次）→ 400「密码错误次数过多，请15分钟后再试」；
2. 验证码校验：过期/未输入 → 400「验证码已过期，请刷新」；不匹配 → 400「验证码错误」（校验后无论成败均删除，防止重放）；
3. 账号密码校验：失败 → 400「用户名或密码错误」，同时累计失败次数；成功则清除失败计数并签发 JWT。
...
`POST /auth/logout`　权限：登录用户

说明：将当前请求携带的令牌写入 Redis 黑名单（key `jwt:blacklist:{token}`），过期时间为令牌剩余有效期；`JwtInterceptor` 对后续请求检查黑名单，命中返回 401，实现"退出后令牌立即失效"（弥补 JWT 无状态、签发后无法撤销的缺陷）。前端收到成功响应后清除本地登录态。
