...
| 缓存中间件 | Redis 7.x | 高性能内存数据库，支持 key 自动过期；承载验证码、登录失败计数、令牌黑名单等"短生命周期"数据 |
| 认证 | JWT（jjwt） | 无状态令牌，服务端不存会话 |
| 验证码 | Hutool（hutool-captcha） | 一行代码生成干扰线图形验证码，图片 Base64 输出 |
...
├── common/          # 统一响应 Result、错误码、BusinessException、全局异常处理、RedisService（Redis 操作封装）
├── config/          # WebConfig（拦截器注册、CORS）、MybatisPlusConfig（分页插件）
├── security/        # JwtUtil、JwtInterceptor、RequireRole 注解、UserContext（角色校验在拦截器中完成）
├── aspect/          # LogAspect（操作日志切面）
├── controller/      # AuthController、UserController、LogController、ElderController、
│                    #   CareRecordController、HealthRecordController、MedicineController（用药计划/任务）、
...
```
src/
├── api/             # auth.js、user.js、elder.js、care.js、health.js、medicine.js、
│                    #   visit.js、message.js、stats.js、log.js（每模块一个文件）
├── styles/theme.css # 全局主题"暖阳关怀"：覆盖 Element Plus CSS 变量（配色/圆角/胶囊标签）
...
└── views/
    ...
    ├── elder/       # ElderList.vue（新增/编辑为页内弹窗）
...
### 2.2 JWT 认证与权限控制（含 Redis 登录安全设计）

**登录流程**：获取图形验证码 → 登录接口依次校验【账号是否被锁定 → 验证码 → 账号密码（BCrypt 校验）】→ 签发 JWT（payload 含 userId、username、role，有效期 24h）→ 前端保存。

**Redis 登录安全三件套**（key 设计汇总）：

| Redis key | 值 | 过期时间 | 用途 |
|-----------|-----|---------|------|
| `captcha:{captchaId}` | 验证码文本（忽略大小写比对） | 5 分钟 | 图形验证码；校验一次后立即删除，防重放 |
| `login:fail:{username}` | 失败次数（INCR 累加） | 首次失败起 15 分钟 | 密码错误计数；达 5 次锁定，登录成功即清除 |
| `jwt:blacklist:{token}` | 1 | 令牌剩余有效期 | 登出黑名单；拦截器命中即返回 401，弥补 JWT 签发后无法撤销的缺陷 |

三项数据的共同特征是"**短生命周期、高频读写、可容忍丢失**"——正适合放 Redis 而不是 MySQL（无需建表、自动过期、性能高）。

**鉴权链路**：

1. `JwtInterceptor`（实现 HandlerInterceptor）注册到 `/api/**`，放行 `/api/auth/login`、`/api/auth/captcha`；
2. preHandle 解析 `Authorization: Bearer <token>` → JwtUtil 校验签名与过期时间 → 检查 Redis 黑名单（`jwt:blacklist:{token}` 存在则 401）→ 从 token 取出 userId/role 存入 ThreadLocal（`UserContext`）；
...
**安全退出**：`POST /auth/logout` 将当前令牌写入黑名单后，前端再清除本地登录态；被锁定的账号在锁定期间即使密码正确也拒绝登录。验证码、失败计数、黑名单统一通过 `RedisService`（StringRedisTemplate 封装）读写，验证码生成用 Hutool 的 `LineCaptcha`（4 位字符、干扰线，图片以 Base64 data URL 返回前端展示）。

### 2.3 操作日志（AOP）

- 自定义注解 `@OperLog("新增老人")` 标注在 Controller 写操作方法上；
...
- 角色不匹配时跳回该角色默认首页（未单独设 403 页面）；菜单按角色动态过滤显示。
...
为避免与常见后台模板同质化，前端通过 `styles/theme.css` 建立统一的视觉体系，全部基于 CSS 变量实现，一处定义、全站生效（含 Element Plus 组件换肤）：

| 设计要素 | 约定 |
|----------|------|
| 色彩体系 | 奶油底色 `#f6f2ea` + 柿橙主色 `#d96f3a`（暖阳/夕阳意向）+ 墨绿辅助 `#2f5d50`（健康守护）+ 暖棕墨文字 |
| 字体 | 界面标题、品牌名、卡片标题使用宋体系衬线字体（STZhongsong/SimSun），正文用系统无衬线字体，无外部字体依赖 |
| 组件风格 | 16px 圆角卡片、胶囊形标签与消息提示、米色表头表格、圆角按钮与分页 |
| 布局 | 侧边栏米色菜单（按角色过滤，选中项暖橙胶囊），顶栏毛玻璃效果带角色徽章与用户头像 |
| 登录页 | 居中圆角卡片：品牌标识 + 衬线标题 + 标语、登录表单；演示账号胶囊点击自动填充 |
| 图表配色 | 护理/收缩压暖橘、探访/舒张压墨绿，年龄分布环形图暖色系 |
...
| 认证安全 | JWT 24h 过期；实现统一拦截；登录失败统一提示（不暴露用户是否存在）；图形验证码防机器批量尝试；连续错 5 次锁定 15 分钟（Redis 计数）；登出后令牌写入 Redis 黑名单立即失效 |
| 缓存安全 | 验证码一次性使用（校验即删）；失败计数达上限即锁定；黑名单 TTL 与令牌剩余有效期对齐，不产生持久垃圾数据 |
...
- MySQL 8.0（本地或服务器）
- Redis 7.x（本地或服务器；Windows 环境可用 GitHub 开源 Windows 移植版 / Memurai / Docker 运行，默认 `localhost:6379` 无密码）
- JDK 17+（Spring Boot 3.x 要求）
- Node 16+（仅开发构建需要，前端可构建后由后端/静态服务器托管）

### 6.2 启动步骤

1. 启动 Redis（默认 `localhost:6379`，连接信息在 `application.yml` 的 `spring.data.redis` 修改）
2. 导入初始化脚本（自带建库语句与演示数据，演示日期按导入当日生成）：`mysql -u root -p < init.sql`
3. 后端：修改 `application.yml` 数据源和 JWT 密钥 → `mvn spring-boot:run`（端口 8080）
4. 前端：`npm install` → `npm run dev`（开发环境配置 Vite 代理 `/api` → `http://localhost:8080`；`npm run preview` 可本地验证打包产物，同样已配置代理）
5. 访问 `http://localhost:8081`（Vite 默认端口），默认管理员 `admin / 123456`（需输入图形验证码）
...
graph TB
    ...
    subgraph 数据层[数据层]
        ...
        REDIS[(Redis 7.x<br/>验证码/失败计数/令牌黑名单)]
    end
